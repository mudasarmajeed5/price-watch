...
import { Home, Eye, Search, BarChart2, User, TrendingDown, Minus, Bell } from "lucide-react";
...
const watchlist = [
  { id: 1, name: "iPhone 15 Pro 256GB", brand: "Apple", price: 385000, drop: 5000, image: "https://images.unsplash.com/photo-1696446701796-da61225697cc?w=500&q=80" },
  { id: 2, name: "Dell XPS 13 Plus", brand: "Dell", price: 420000, drop: 0, image: "https://images.unsplash.com/photo-1593640408182-31c70c8268f5?w=500&q=80" },
  { id: 3, name: "Samsung Galaxy Tab S9", brand: "Samsung", price: 198000, drop: 8000, image: "https://images.unsplash.com/photo-1544244015-0df4b3bd43cb?w=500&q=80" },
  { id: 4, name: "Sony WH-1000XM5", brand: "Sony", price: 89000, drop: 0, image: "https://images.unsplash.com/photo-1618366712010-f4ae9c647dcb?w=500&q=80" },
];

const recentlyAdded = [
  { id: 1, name: "Sony PlayStation 5 Disc Edition", store: "Daraz", discount: 12, current: 165000, original: 188000, addedAt: new Date("2026-05-24"), image: "https://images.unsplash.com/photo-1606813907291-d86efa9b94db?w=500&q=80" },
  { id: 2, name: "AirPods Pro (2nd Gen)", store: "PriceOye", discount: 8, current: 62500, original: 68000, addedAt: new Date("2026-05-23"), image: "https://images.unsplash.com/photo-1600294037681-c80b4cb5b434?w=500&q=80" },
      ...
      <main className="flex-1 overflow-y-auto pb-20">
        ...
        <section className="pt-5">
          <div className="flex items-center justify-between px-3 mb-3">
            ...
            <Link href="/watchlist" className="text-xs text-emerald-700 font-medium">View All</Link>
          </div>
          <Carousel opts={{ align: "start", dragFree: true }} className="w-full pb-2">
                  ...
                  <Card className="rounded-xl border shadow-none">
                    <CardContent className="p-3">
                      <div className="relative w-full h-32 rounded-xl bg-emerald-50 overflow-hidden mb-3">
                        <Image src={item.image} alt={item.name} fill className="object-cover" sizes="(max-width: 768px) 75vw, 300px" />
                      </div>
                      <div className="flex flex-col gap-0.5">
                        ...
                        <div className="mt-1.5 flex items-center justify-between">
                          <p className="text-base font-bold text-emerald-700">Rs. {item.price.toLocaleString()}</p>
                          {item.drop > 0 ? (
                            <Badge className="bg-emerald-50 text-emerald-700 hover:bg-emerald-50 border-0 text-[10px] px-1.5 py-0.5 gap-1">
                              <TrendingDown size={12} /> Rs. {item.drop.toLocaleString()}
                            </Badge>
                          ) : (
        ...
        <section>
          <div className="flex items-center justify-between px-3 mb-4">
            ...
            <Link href="/deals" className="text-xs text-emerald-700 font-medium">View All</Link>
          </div>
              ...
              <Card key={deal.id} className="rounded-xl border shadow-none">
                <CardContent className="p-2 py-0 flex items-center gap-3 relative">
                  <div className="relative w-16 h-16 rounded-lg bg-emerald-50/50 shrink-0 overflow-hidden">
                    <Image src={deal.image} alt={deal.name} fill className="object-cover" sizes="64px" />
                  </div>
                  <div className="flex-1 min-w-0 pr-8">
                    <p className="text-xs font-semibold leading-snug mb-1 line-clamp-1">{deal.name}</p>
                    ...
                    <div className="flex items-baseline gap-1.5">
                      <span className="text-sm font-bold text-emerald-700">Rs. {deal.current.toLocaleString()}</span>
                      <span className="text-[10px] text-muted-foreground line-through">Rs. {deal.original.toLocaleString()}</span>
                    </div>
                  </div>
